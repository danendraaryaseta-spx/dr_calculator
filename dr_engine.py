...
TAB = dict(
    fm="FM Projection", lm="LM Projection", sched="schedule_v5",
    active="Active DC", units="data_unit_nasional",
)
HEADER_ROW = {"schedule_v5": 3, "worksheet inner": 6, "data_unit_nasional": 2}

SCOL = dict(origin="Origin", scheme="LH Scheme", segment="LH by Distance",
            cap_lm="Cap LM", cap_fm="Cap FM", dt="DT VALIDATION",
...
fm_dst=col(fm,"Destination Station Name","Destination"); fm_dt=col(fm,"Fcst Date","Date"); fm_ado=col(fm,"Forecast ADO","ADO")
lm_org=col(lm,"Start Station Name","Origin","Start"); lm_dt=col(lm,"Fcst Date","Date"); lm_ado=col(lm,"Forecast ADO","ADO")
...
s = sched.copy()
sc_org=SCOL["origin"]; sc_scheme=SCOL["scheme"]; sc_seg=SCOL["segment"]
sc_clm=SCOL["cap_lm"]; sc_cfm=SCOL["cap_fm"]; sc_dt=SCOL["dt"]

for c in [sc_clm, sc_cfm]:
    s[c] = pd.to_numeric(s[c].astype(str).str.replace(",","", regex=False), errors="coerce").fillna(0)

mask = (s[sc_scheme].astype(str).str.strip() != "X") \
...
if "Facility name" not in u.columns:
    for idx in range(min(5, len(u))):
        if "Facility name" in u.iloc[idx].values:
            ...
            break
    u.columns = [str(c).strip() if str(c).strip() != "" else f"_col{i}" for i,c in enumerate(u.columns)]
...
ideal = (cnt * 3 * PARCELS_PER_TRUCK * IDEAL_FACTOR).rename("cap_ideal").reset_index().rename(columns={u_dc: "dc"})
...
try:
    mp = load("mapping")
    mp_dc = "Setting"; mp_days = "days"; mp_time = "time"
    for _, row in mp.iterrows():
        ...
        if dc and dc != "Setting":
            ...
            # find matching records and add mapping data
            for r in records:
                if r["dc"] == dc:
                    r["offset"] = offset
                    r["time"] = time_val
...
for dc, sub in df.groupby("dc"):
    ...
    a0 = trucks(wid, TRIPS_LM); a1 = trucks(wv5, TRIPS_LM)
